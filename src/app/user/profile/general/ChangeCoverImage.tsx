...
const ChangeCoverImage = ({ cookie, username, setResponse }: { cookie: string; username: string; setResponse: React.Dispatch<React.SetStateAction<any>>; }) => {
    ...
    <label className="flex justify-center gap-2  rounded-lg my-2 p-2 font-bold items-center">
      {res?.res.image ? (
        <img
          src={imgSrc}
          alt="Selected Image"
          className="w-[100px] h-[100px] my-2 hover:cursor-pointer"
          onClick={() => ref.current?.click()}
        />
      ) : (
        <div className="flex justify-center items-center text-center w-[100px] bg-gray-200 h-[100px]">
